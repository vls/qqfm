...
MUSIC.module.webPlayer.interFace = (function () {
    ...
    function startPlayer() { !! webPlayer && webPlayer.startPlayer();
    }
    ...
    function setPlayerList(isPlay, arr, isLoop) { !! playerList || (playerList = g_playerList());
...
MUSIC.module.webPlayer.qqPlayer = function (fromTag) {
    ...
    var mQQPlayerConfig = {
        ...
        REP_PLAYSONG_IP_ARRAY: ["58.60.11.85", "121.14.96.113", "58.61.165.50", "121.14.78.75"],
        ...
        showPage: function (pageNum) {
            if (pageNum > this.maxPage || pageNum < 1) {} else {
                ...
                var i = (pageNum - 1) * this.itemNum,
                    ...
                    listHTML = [],
                    itemHTML = "",
                    item, radioNameList = g_fmChn._fmNameObj;
                for (; i < len; i++) {
                    if (i < maxIndex) {
                        item = this.idList[i];
        ...
        showPage: function (pageNum) {
            if (pageNum > this.maxPage || pageNum < 1) {} else {
                ...
                var i = (pageNum - 1) * this.itemNum,
                    ...
                    listHTML = [],
                    itemHTML = "",
                    item, maxIndex = this.songList.length;
                for (; i < len; i++) {
                    if (i < maxIndex) {
        ...
        playAll: function () {
            if (this.songList.length > 0) {
                var songinfo_list = [],
                    ...
                    end = this.currentPage * this.itemNum,
                    maxIndex = this.songList.length,
                    tmp;
                ...
                if (songinfo_list.length > 0) {
                    this.OpenPostWindow("http://qzone-music.qq.com/fcg-bin/yqqcom_play_mid.fcg", "_webQQMusicPlayer", {
                        songList: this.getPostData(songinfo_list)
                    });
                } else {
                    return;
                }
